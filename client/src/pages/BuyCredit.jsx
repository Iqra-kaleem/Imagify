...
const BuyCredit = () => {
  ...
  const{user, backendUrl, loadCreditsdata, token,
    setShowLogin } = useContext(AppContext);
    ...
    const paymentRazorpay = async (planId)=>{
       try {
        ...
        console.log("Calling backend with planId:", planId);
        console.log("Sending to backend:", { userId: user?._id, planId });
      ...
      {plans.map((item, index)=>(
        ...
        duration-500' key={index}>
          ...
          <p className='mt-6'>
            ...
            <button onClick={()=>paymentRazorpay(item.id)} className='w-full bg-gray-800 text-white
            mt-8 text-sm rounded-md py-2.5 min-w-52 cursor-pointer
